...
export default function PageShell({ title, subtitle, headerAction, children, fullWidth = false, hideSector = false, className = '' }) {
  return (
    <div
      className={[
        fullWidth ? 'relative w-full max-w-none space-y-6' : 'relative mx-auto max-w-5xl space-y-6',
        className,
      ]
        .filter(Boolean)
        .join(' ')}
