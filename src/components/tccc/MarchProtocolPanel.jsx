    ...
    >
      <div className="mb-3 flex items-start justify-between gap-2">
        <div>
          ...
          <p className="font-mono-technical text-sm font-bold text-white">{step.title}</p>
          <p className="font-mono-technical text-xs text-slate-400">{step.subtitle}</p>
        </div>
        <button
          type="button"
          onClick={onClose}
          className="rounded-lg border border-white/15 p-1.5 text-slate-500 transition hover:text-[#ffb400]"
          aria-label="Kapat"
        >
      ...
      <p className="mb-4 font-mono-technical text-[11px] leading-relaxed text-slate-200">{detail.definition}</p>

      <ul className="space-y-2">
        {detail.protocols.map((line) => (
          <li
            key={line}
            className="rounded-lg border border-white/8 bg-black/35 px-3 py-2 font-mono-technical text-[10px] leading-relaxed text-slate-300"
          >
      ...
      <p className="mt-4 rounded-lg border border-white/8 bg-black/40 px-3 py-2.5 font-mono-technical text-[10px] leading-relaxed text-slate-500">
        {step.doctrine}
      </p>
    </div>
  )
