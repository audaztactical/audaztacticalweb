...
import { ifakCategoryOptions, labelIfakCategory, labelIfakStatus } from '../../lib/healthDisplayText'
import { IFAK_INITIAL_ITEM_FORM } from '../../lib/tcccHealthConstants'
...
export default function TcccIfakInventoryTab({
  ...
  const categories = ifakCategoryOptions()
        ...
        <TacticalPanel className="relative z-[1] w-full max-w-lg border-accent/20 bg-app-bg/98 p-0 shadow-2xl backdrop-blur-md">
          <div className="border-b border-white/10 bg-app-bg px-3 py-2 sm:px-4">
            <p className="font-mono-technical text-[10px] font-bold uppercase tracking-[0.28em] text-accent/90">
              {editingId ? t('ifak.modal.editTitle') : t('ifak.modal.createTitle')}
            </p>
            <p className="mt-0.5 font-mono-technical text-[7px] uppercase text-app-text/45">
              {t('ifak.modal.subtitle')}
            </p>
          </div>
          <form onSubmit={handleSubmit} className="flex flex-col gap-4 px-3 py-4 sm:px-4">
            <label className="block w-full">
              <span className={labelClass}>{t('ifak.modal.itemName')}</span>
              <input
                className={inputClass}
                placeholder={t('ifak.modal.itemNamePlaceholder')}
                value={form.itemName}
            ...
            <label className="block w-full">
              <span className={labelClass}>{t('ifak.modal.category')}</span>
              <select
                value={form.category}
            ...
            <div className="grid w-full gap-4 sm:grid-cols-2">
              <label className="block min-w-0">
                <span className={labelClass}>{t('ifak.modal.quantity')}</span>
                <input
                  type="number"
                  min={0}
                  step={1}
              ...
              <label className="block min-w-0">
                <span className={labelClass}>{t('ifak.modal.expiration')}</span>
                <input
                  type="date"
  ...
  return (
    <section aria-label={t('ifak.ariaLabel')}>
      ...
      {loading && rows.length === 0 ? (
        ...
        <div className="flex min-h-[160px] flex-col items-center justify-center rounded-xl border border-dashed border-accent/25 bg-black/40">
          ...
          <p className="mt-3 font-mono-technical text-[10px] uppercase text-app-text/45">{t('ifak.empty')}</p>
        </div>
      ) : (
            ...
            return (
              ...
              >
                ...
                <p className="truncate pr-12 font-mono-technical text-[9px] uppercase text-app-text/55">
                  {labelIfakCategory(toStr(row.category))}
                </p>
                ...
                <div className="mt-3 flex items-end justify-between gap-4 font-mono-technical tabular-nums">
                  ...
                  <div className="text-right">
                    <p className="text-[9px] uppercase text-app-text/45">{t('ifak.skt')}</p>
                    <p className="text-sm">{toStr(row.expirationDate).replace(/-/g, '.')}</p>
                    <p className="mt-0.5 text-[9px] font-bold uppercase">{labelIfakStatus(st)}</p>
                  </div>
                </div>
              </div>
            )
