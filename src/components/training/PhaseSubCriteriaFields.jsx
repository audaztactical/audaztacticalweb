...
import {
  formatObservedEvalAriaScoreButton,
  formatObservedEvalAriaScoreGroup,
  formatObservedEvalScoreRangeLabel,
  formatObservedEvalSelectPlaceholder,
  resolveObservedEvalCriterionLabel,
} from '../../lib/trainingDisplayText'

/** @typedef {import('../../lib/evaluationPhaseCriteria').EvaluationSubCriterion} EvaluationSubCriterion */
/** @typedef {import('../../lib/trainingDisplayText').ObservedEvalDiscipline} ObservedEvalDiscipline */
 ...
 *   discipline?: ObservedEvalDiscipline
 *   phaseId?: string
...
export default function PhaseSubCriteriaFields({
  ...
  const selectPlaceholder = formatObservedEvalSelectPlaceholder()
  ...
  return (
    <div className="space-y-3">
      {criteria.map((criterion) => {
        const displayLabel = resolveObservedEvalCriterionLabel(criterion, discipline, phaseId)
            ...
            {variant === 'select' ? (
              <label className="block space-y-1.5">
                <span className={labelClassName}>
                  {formatObservedEvalScoreRangeLabel(displayLabel, min, max)}
                </span>
                <select
...
function SegmentedCriterionBar({
  ...
  return (
    <div className="space-y-1.5" role="group" aria-label={formatObservedEvalAriaScoreGroup(label)}>
        ...
        {values.map((n) => {
          ...
          return (
            <button
              ...
              aria-label={formatObservedEvalAriaScoreButton(label, n)}
 ...
 * }} props
 */
export function PhaseSubScoresDisplay({
  phaseData,
  criteria,
  maxScore = 10,
  compact = false,
    ...
    return (
      <ul className={compact ? 'space-y-1' : 'space-y-2'}>
        {criteria.map((c) => {
          ...
          const displayLabel = resolveObservedEvalCriterionLabel(c, discipline, phaseId)
          return (
            <li
              key={c.id}
              className={
            ...
            >
              <span className={compact ? 'text-zinc-500' : 'text-xs text-zinc-500'}>{displayLabel}</span>
