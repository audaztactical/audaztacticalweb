...
export default function IlwsCategoryHubCard({ title, onEnter, modelVariant, imageSrc, imageAlt }) {
  return (
    <button
      type="button"
      onClick={onEnter}
      className="group flex w-full flex-col overflow-hidden rounded-lg border border-white/10 bg-black/40 text-left shadow-[inset_0_1px_0_rgba(255,255,255,0.04)] transition-[border-color,box-shadow] duration-300 hover:border-[#00FF41]/35 hover:shadow-[0_0_24px_-8px_rgba(0,255,65,0.35)] focus:outline-none focus-visible:ring-1 focus-visible:ring-[#00FF41]/50"
    >
      <h2 className="shrink-0 border-b border-white/10 bg-[#080808] px-4 py-4 text-center font-display text-base font-bold uppercase tracking-[0.18em] text-white sm:text-lg">
        {title}
      </h2>
