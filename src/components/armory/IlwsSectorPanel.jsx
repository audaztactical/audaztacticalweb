...
export default function IlwsSectorPanel({
  ...
  return (
    <TacticalPanel className="flex min-h-0 flex-col overflow-hidden border-white/10 bg-black/35 p-0">
      <div className="shrink-0 border-b border-white/10 bg-app-bg px-3 py-2">
        <p className="font-mono-technical text-[9px] font-bold uppercase tracking-[0.28em] text-accent/85">{title}</p>
        <p className="mt-0.5 font-mono-technical text-[7px] uppercase tracking-wider text-app-text/45">{subtitle}</p>
      </div>
      ...
      <div className="min-h-0 flex-1 space-y-2 overflow-y-auto p-3 op-detay-col-scroll max-h-[28rem]">
        {rows.length === 0 ? (
          <p className="py-6 text-center font-mono-technical text-[9px] uppercase tracking-widest text-app-text/45">{emptyHint}</p>
        ) : (
          rows.map((row) => (
            <IlwsInventoryCard
              key={row.id}
