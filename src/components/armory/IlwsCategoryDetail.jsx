...
export default function IlwsCategoryDetail({
  ...
  return (
    <TacticalPanel className="border-white/10 bg-black/35 p-0">
      <div className="border-b border-white/10 bg-[#080808] px-3 py-3 sm:px-4">
        <button
          type="button"
          onClick={onBack}
          className="mb-3 inline-flex items-center gap-2 rounded border border-[#ffb400]/50 bg-[#ffb400]/12 px-3 py-2 font-mono-technical text-[9px] font-bold uppercase tracking-wider text-[#ffb400] shadow-[0_0_12px_-4px_rgba(255,180,0,0.4)] transition hover:bg-[#ffb400]/20"
        >
          <span aria-hidden>↩️</span>
          GERİ DÖN / RETURN
        </button>
        <p className="font-display text-base font-bold uppercase tracking-[0.14em] text-white sm:text-lg">{title}</p>
      </div>

      <div className="max-h-[min(70vh,42rem)] space-y-3 overflow-y-auto p-3 op-detay-col-scroll sm:p-4">
        {rows.length === 0 ? (
          <p className="py-12 text-center font-mono-technical text-[10px] uppercase tracking-widest text-slate-600">{emptyHint}</p>
        ) : (
